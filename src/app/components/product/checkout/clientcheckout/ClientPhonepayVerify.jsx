...
export default function ClientPaymentVerifyPhonepe() {
  ...
  const [clientToken, setClientToken] = useState(sessionStorage.getItem("tempClientToken"));
    ...
    if (paymentStatus === "success") {
      const interval = setInterval(() => {
        ...
        if (time === 1) {
          ...
          const cliToken = sessionStorage.getItem("tempClientToken");
  ...
  const fetchOrderdetails = async () => {
    try {
      ...
      const response = await axios.get(
        `https://admin.tradingmaterials.com/api/client/product/checkout/view-order?order_id=${orderID}&client_id=${sessionStorage.getItem(
          "tempClientId"
        )}`,
        {
          headers: {
            Authorization:
              "Bearer " + sessionStorage.getItem("tempClientToken"),
          },
        }
      );
  ...
  //payment verification phonepe
  async function verifyPayment() {
    const token = sessionStorage.getItem("tempClientToken");
    sessionStorage.setItem("order_id", id);

    try {
      setPaymentVerification(true);
      const phonepeordDI = sessionStorage.getItem("phonepeOrdId");

      const response = await axios.post(
        "https://admin.tradingmaterials.com/api/client/product/checkout/verify-payment",
        {
          order_id: orderID,
          payment_id: phonepeordDI,
          payment_type: "Phonepe",
          client_id: sessionStorage.getItem("tempClientId"),
        },
        {
          headers: {
                ...
                {paymentStatus !== "loading" && paymentStatus == "failed" && (
                  <div data-aos="fade-down">
                    ...
                    <div className="flex justify-center items-center my-2">
                      <Button
                        variant="contained"
                        href={`/client/checkout/${CryptoJS?.AES?.encrypt(
                          `${orderID}`,
                          "trading_materials_order"
                        )
                          ?.toString()
                          .replace(/\//g, "_")
                          .replace(/\+/g, "-")}/${sessionStorage.getItem(
                          "tempClientToken"
                        )}/${sessionStorage.getItem("tempClientId")}`}
                        target="_blank"
                        type="button"
                    ...
                    >
                      {" "}
                      <BsFillPlayFill /> Back to &ldquo;My Dashboard&ldquo;
                    </p>
                  </div>
                )}
                {paymentStatus !== "loading" && paymentStatus != "failed" && (
