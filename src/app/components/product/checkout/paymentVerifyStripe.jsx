...
export default function PaymentVerifyStripe() {
  ...
  //               `${localStorage.getItem("client_token")}`, "order_details"
  //             )?.toString()
  //             .replace(/\//g, "_")
  //             .replace(/\+/g, "-")}`, "_blank")
  //         } else {
  //           navigate(`/orders/${CryptoJS?.AES?.encrypt(
  //               `${clientToken}`, "order_details"
  //             )?.toString()
  //             .replace(/\//g, "_")
  //             .replace(/\+/g, "-")}`, "_blank")
  //         }
  ...
  const fetchOrderdetails = async () => {
    try {
      ...
      const response = await axios.get(
        `https://admin.tradingmaterials.com/api/client/product/checkout/view-order?order_id=${localStorage.getItem("id")}&client_id=${userData?.client?.id}`,
        {
          headers: {
